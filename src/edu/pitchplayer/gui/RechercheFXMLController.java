...
//    if (terrainSelectionne != null) {
//        try {
//            String terrainNom = terrainSelectionne.getNom();
//            String imageUrl = getTerrainImageUrl(terrainNom);
...
public class RechercheFXMLController implements Initializable {
    ...
    private void TrieAction(ActionEvent event) {
     ...
     ObservableList<Terrain> items = fxView.getItems();
    if (!items.isEmpty()) {
        // Tri par nom de terrain en ordre alphabétique croissant
        items.sort(Comparator.comparing(Terrain::getNom, Comparator.nullsLast(String::compareToIgnoreCase)));

        // Mettre à jour la ListView avec les données triées
        fxView.refresh();
    }
    }
